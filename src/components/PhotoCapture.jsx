...
export default function PhotoCapture({ open, onClose, onProduct }) {
  ...
  const captureAndRecognise = async () => {
    ...
    try {
      // Use Tesseract.js to read text from the image
      ...
      if (text && text.trim().length > 2) {
        // Try to find the most likely product name from the OCR text
        const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 2);
        // Pick the line that looks most like a product name (not numbers, not too short)
        const productLine = lines.find(l => /[a-zA-Z]{3,}/.test(l) && !/^\d+$/.test(l)) || lines[0];

        if (productLine) {
          onProduct({
            name: productLine.replace(/[^a-zA-Z0-9\s'-]/g, '').trim(),
            source: 'photo'
          });
          // Stop camera
          if (streamRef.current) {
          ...
          setError('Could not identify product from photo. Try typing the name instead.');
        }
      } else {
  ...
  return (
    <div className="camera-view">
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 20, fontWeight: 800 }}>Photo Identify</h2>
