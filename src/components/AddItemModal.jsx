...
export default function AddItemModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      // If we opened from a Scan/Photo flow, skip the search step and go
      // straight to the store picker with brand/weight already populated.
      if (initialProduct?.name) {
        const name = initialProduct.name;
        const brand = initialProduct.brand || initialProduct.brands || '';
        setSelectedName(name);
        setSelectedBrand(brand);
        setSelectedWeight(extractWeight(name));
        setStore(
          initialProduct.suggested_store ||
          getSuggestedStore(name)
        );
        setStep('store');
      } else {
  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      // 2. Open Food Facts v2 API - AU-filtered, popularity-sorted (fast)
      // NOTE: au.openfoodfacts.org only changes UI language; we must pass
      // countries_tags_en=australia to actually filter AU products. We also
      // sort by popularity_key so Coles/Woolies/Aldi staples surface first.
      //
      // RELEVANCE: the v2 search API combined with popularity_key sort tends
      // to return popular AU products regardless of text relevance (e.g.
      // Weet-Bix when searching "milk"). We fetch a larger page and filter
      // client-side so every suggestion actually contains the typed terms.
      try {
        const url =
        ...
        if (!cancelled && data.products && data.products.length) {
          // Split query into terms; every term must appear in the product's
          // name, brand, or category tags for the product to be considered
          // relevant. This kills irrelevant popular products.
          ...
          const matches = data.products.filter(p => {
            ...
            const haystack = `${name} ${brands} ${cats}`;
          ...
          for (const p of matches) {
            ...
            if (!raw) continue;
            // Append quantity if not already in the name (e.g. "Milk 1L")
            let display = raw;
            if (p.quantity && !raw.toLowerCase().includes(p.quantity.toLowerCase())) {
        ...
        // OFF unavailable / aborted - fall through with whatever we have
      }
  ...
  // immediately. We pipe brand + weight through so the Gatekeeper can build
  // a tight SerpApi query (e.g. "Campos King St Coffee 250g") instead of
  // getting a thousand irrelevant matches.
  const handleAddItem = () => {
